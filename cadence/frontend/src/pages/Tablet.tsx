        ...
        <header className="hud-top">
          <div className="now">
            ...
            <div className="next">
              {st?.next_chapter ? (
                <>
                  Next <b>{st.chapter_hold ? `when ${(st.chapter_hold.label ?? st.chapter_hold.entity_id).replace(/^binary_sensor\./, "").replace(/ \(group\)$/, "")} clears${st.chapter_hold.until ? ` (by ${fmtTime(st.chapter_hold.until)})` : ""}` : st.next_chapter.pending_condition ? `waiting for ${st.next_chapter.kind}` : fmtTime(st.next_chapter.at)}</b> {st.next_chapter.name}
                </>
              ) : null}
